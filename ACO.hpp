...
public:
    ...
    double alpha = 1, beta = 1.0, rho = 0.05;
...
std::vector<int> ACO::choose_trucks(std::vector<Tour> &tours, std::vector<int> &available_trucks, int n_trucks) {
    ...
    for(int i = 0; i < int(w.size()); ++i) w[i] = 1.0 / (tours[ available_trucks[i] ].tot_demand + 1);
    for(int i = 0; i < n_trucks; ++i) {
...
void ACO::fix_solution(Solution &sol) {
    ...
    while (true) {
        ...
        bool improved = false;
        for(int i = 0; i < int(sol.tours.size()); ++i)
            for(int j = i + 1; j < int(sol.tours.size()); ++j) {
                ...
                if (ok) {
                    ...
                    LS::ls_tour(sol.tours[i], *instance);
                    LS::ls_tour(sol.tours[j], *instance);
